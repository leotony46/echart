...
console.log("【实例五】准备打开已存在的文件！");
fs.open('input.txt', 'r+', function(err, fd) {
    if (err) {
        return console.error(err);
    }
    console.log("【实例五】文件打开成功！");
    console.log("【实例五】准备读取文件：");
    fs.read(fd, buf, 0, buf.length, 0, function(err, bytes){
        if(err){
            console.log(err);
        }
        console.log("【实例五】" + bytes + "  字节被读取");
        
        // 仅输出读取的字节
        if(bytes > 0){
            console.log("【实例五】" + buf.slice(0, bytes).toString());
        }
    });
});
...
fs.open('input.txt', 'r+', function(err, bytes){
    if(err){
        console.log(err);
    }
    ...
    fs.read(fd, buf, 0, buf.length, 0, function(err, bytes){
        ...
        // 仅输出读取的字节
        if(bytes > 0){
            console.log("【实例六】" + buf.slice(0, bytes).toString());
        }
